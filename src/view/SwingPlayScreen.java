...
public class SwingPlayScreen extends JFrame implements GamePlayView {
  ...
  private JPanel createBoardPanel() {
    ...
    ImageIcon boardIcon = new ImageIcon(
        getClass().getResource("/view/images/" + boardType.toString() + ".png"));
  ...
  private JPanel createPlayerInfoPanel() {
    ...
    for (int i = 0; i < playerCount; i++) {
      ...
      ImageIcon pieceIcon = new ImageIcon(
          getClass().getResource("/view/images/플레이어" + playerId + "말.png"));
  ...
  public void updateCurrentPlayer(int playerNumber) {
    ...
    for (int i = 0; i < playerCount; i++) {
      ...
      for (Component comp : components) {
        if (comp instanceof JLabel && comp != pieceLabels.get(i)
            && ((JLabel) comp).getIcon() != null) {
  ...
  public void repaintAllPieces() {
    ...
    for (Map.Entry<Integer, List<PositionDTO>> entry : cellPieces.entrySet()) {
      ...
      if (piecesAtCell.size() == 1) {
        ...
        if (pieceLabel != null) {
          if (boardType == BoardType.SQUARE) {
            ...
            pieceLabel.setBounds(pos.x - 12, pos.y - 25, 40, 40);
          } else {
            pieceLabel.setBounds(pos.x - 8, pos.y - 35, 40, 40);
          }
      ...
      // 말이 여러 개일 경우 (업혀있는 경우)
        ...
        JLabel mainPieceLabel = playerPiecesMap.get(
        ...
        if (mainPieceLabel != null) {
          if (boardType == BoardType.SQUARE) {
            ...
            mainPieceLabel.setBounds(pos.x - 14, pos.y - 25, 40, 40);
          } else {
            mainPieceLabel.setBounds(pos.x - 8, pos.y - 35, 40, 40);
          }
