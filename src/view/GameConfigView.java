...
public interface GameConfigView {
   ...
   * SwingView 가 할당될 수도 있고, JAVAFXView 가 할당될 수도 있다는 것. Controller 가 각기 다른 UI 에 대해 "컨트롤러 코드의 수정 없이"
   * 작업을 수행해야함. 게임 설정 화면을 예시로 들어보자. swing 에서는 combobox 를 통해 사용자 입력을 받음. 근데. javafx 에서는 어떤 방식인지 모름
   * 그렇기 때문에 인터페이스에서 사용자 입력을 얻어오는 방식, 오류 메시지 표시 방식, 뷰를 닫는 방식 등을 추상화 해놓고 Swing 화면과 Javafx 화면을 제작할때 이
   * 메소드들을 구체적으로 구현하는 것 .
   */
